	...
	DOUBLE PRECISION :: xi1(nfrs),xi2(nfrs),xmin,xmax,ymin,ymax,proj1,proj2,struct(nfrs,2),dist(nfrs)
	...
	DO i=1,ncoords
	  ...
	  aa=adjustl(aa)
	  WRITE(*,*)TRIM(aa)
	  OPEN(unit=24,file='struct_'//TRIM(bb)//'.ndx',status='unknown')
	  WRITE(24,'(A)')'[ State '//TRIM(aa)//']'
	  counter=0
	  DO k=1,nfrs !counter-1
	    dist(k)=SQRT((coords(i,1)-xi1(k))**2+(coords(i,2)-xi2(k))**2)
	  END DO
	  minframe=minloc(dist)
	  WRITE(*,*)minframe
	  WRITE(24,*)minframe
	  CLOSE(24)
	END DO
