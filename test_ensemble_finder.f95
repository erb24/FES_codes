	...
	DOUBLE PRECISION :: xi1(nfrs),xi2(nfrs),xmin,xmax,ymin,ymax,proj1,proj2,binx,biny
	...
	!structures from the projections of the trajectory.
        OPEN(unit=25,file='frames_'//TRIM(bb)//'.ndx',status='unknown')
        OPEN(unit=26,file='nframes_'//TRIM(bb),status='unknown')
	WRITE(25,'(A)')'[ State 1 ]'
	DO i=1,ncoords
          counter=0
	  WRITE(aa,*)i
	  ...
	  DO k=1,nfrs !counter-1
	    ...
	    !c2bin=1+INT((coords(i,2)/180)*nbins)
            binx=180/nbins !Uniform binsize in the x-direction
            biny=360/nbins !Uniform binsize in the y-direction
	    !IF((ABS((xi1(k)-coords(i,1))/coords(i,1)) .LE. 0.01) .AND. (ABS((xi2(k)-coords(i,2))/coords(i,2)) .LE. 0.01))THEN !Granularity of 0.5% (start here for now)
            IF((ABS(xi1(k)-coords(i,1)) .LE. binx/2) .AND. (ABS(xi2(k)-coords(i,2)) .LE. biny/2))THEN !+/- 0.9 degrees, 1.8 degrees for x, y
              counter=counter+1
	      !WRITE(24,*)k !Frame number
	      WRITE(*,*)k,xi1(k),xi2(k)
              WRITE(25,*)k !What frames belong to the ith coordinate?
	    END IF
	  END DO
          WRITE(26,*)counter !How many frames correspond to each coordinate?
	  !CLOSE(24)
	END DO
        CLOSE(25)
        CLOSE(26)
